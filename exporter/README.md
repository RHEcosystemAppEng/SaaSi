...
```
## Installing the OpenShift service
Container images are automatically generated by the GitHub actions. This command deploys the 2 container application:
```bash
...
```bash
INFRA_EXPORTER_URL=$(oc get route saasi-infra-exporter -ojsonpath='{.spec.host}{.spec.path}')
# Get infra-exporter status
curl -kv "$INFRA_EXPORTER_URL"
# Post export infra request
curl -kv -X POST "$INFRA_EXPORTER_URL" -H "Content-Type: application/json" -d @test.json"
APP_EXPORTER_URL=$(oc get route saasi-app-exporter -ojsonpath='{.spec.host}{.spec.path}')
# Get app-exporter status
curl -kv "$APP_EXPORTER_URL"
# Post export application request
curl -kv -X POST "$APP_EXPORTER_URL" -H "Content-Type: application/json" -d @test.json
