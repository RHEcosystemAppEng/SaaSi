...
import (
	...
	"github.com/RHEcosystemAppEng/SaaSi/deployer/deployer-lib/deployer/infra"
...
func deploy(args *config.Args, logger *logrus.Logger) http.HandlerFunc {
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		if req.Method != POST {
			http.Error(rw, fmt.Sprintf("Expect method %s at %s, got %v", POST, INFRA_DEPLOYER_PATH, req.Method), http.StatusMethodNotAllowed)
			http.NotFound(rw, req)
			return
		}
